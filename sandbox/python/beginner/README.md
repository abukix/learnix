...
- [ ] Bug hunt — a short script with 3-5 deliberately planted bugs (off-by-one in a `range`, `=` instead of `==`, an unwanted mutation, ...) to find and fix via print-statement debugging, not by being told where they are; rehearses CH5's actual debugging process in isolation
