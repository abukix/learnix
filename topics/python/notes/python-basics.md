...
## 12.0 Errors and exceptions in Python
...
## 12.1 Raising exceptions review
...
If you're interested in the official documentation on all the built-in exceptions you can find a list in the [Python docs](https://docs.python.org/3/library/exceptions.html).

**Example**

This code is simply raising an `Exception` with the text `"zero division"`. Multiple `except` clauses are checked top to bottom, and only the first one whose type matches will run.
...
The raised object is a plain `Exception`, not a `ZeroDivisionError` — the string `"zero division"` is just the message, it doesn't change the type. So `except ZeroDivisionError` is skipped (type doesn't match), and it falls through to `except Exception`, which matches because the raised object's type *is* `Exception`. This prints `"other"`.

## 12.3 Exception hierarchy

`except Exception` doesn't just match plain `Exception` objects — it also matches any more specific exception type, because `ZeroDivisionError` (like nearly every built-in exception) is itself a subclass of `Exception`.
...
Here `10 / 0` actually raises a real `ZeroDivisionError` (unlike the previous example, where the type was just `Exception` with a misleading message). Since `ZeroDivisionError` IS-A `Exception`, the `except Exception` clause still matches, and this prints `"other"`.
...
Hovering over a variable that was assigned from a hinted parameter (e.g. a `status` variable set from a hinted `str` parameter) shows the correct inferred type in a tooltip. Without hints, parameters like `name`, `level`, `health`, and `has_magic` all show up as "unknown" — Python can't infer function parameter types on its own; the hints are the only way the tooling learns them.
...
In mathematics, the `!` symbol denotes a factorial, but is not a valid Python operator. Use a loop and multiplication to compute the proper result.
